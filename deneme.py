...
def discretize_dynamics_and_cost(t_horizon, n_points, m_steps_per_point, x, x_ref, u, dynamics_f, cost_f, ind):
    """
    ...
    x0 = x
    q = 0
    x_ref = x_ref + t_horizon*cs.vertcat(x_ref[3:6], [0,0,0])
...
if __name__=="__main__":
    ...
    # x_dot = cs.vertcat(x[3:6], u)
    x_dot = f(x,u)
    Q = cs.diag(q_diag)
    R = cs.diag(r_diag)
    L = (x-x_ref).T @ Q @ (x-x_ref) + u.T @ R @ u 
    # L = cs.cross(x[3:6] , x_ref[3:6])
    
    dynamics_f = cs.Function('x_dot', [x, u], [x_dot], ['x', 'u'], ['x_dot'])
    cost_f = cs.Function('q', [x, u], [L], ['x', 'u'], ['q'])
    F = discretize_dynamics_and_cost(T, N, m_steps_per_point, x, x_ref, u, dynamics_f, cost_f, 0)
    ...
    for k in range(N):
        ...
        if k >= N-1:
            g += [cs.cross(Xk_end[3:6] , x_ref[3:6])]
            lbg += [0]*3
            ubg += [0]*3
            
        if k == N-1:
            g += [Xk_end[0:3]]
            lbg += [target_pos[0]+target_vel[0]*T, target_pos[1]+target_vel[1]*2*T, target_pos[2]+target_vel[2]*2*T]
            ubg += [target_pos[0]+target_vel[0]*T, target_pos[1]+target_vel[1]*2*T, target_pos[2]+target_vel[2]*2*T]
