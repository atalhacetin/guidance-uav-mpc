...
from quad_guidance_optimizer import QuadGuidanceOptimizerCasadi, QuadGuidanceOptimizerAcados
...
def main_acados():
    ...
    for i in range(int(sim_time/dt)):
        ...
        if np.dot(target_pos - x[0:3], target_vel) >= 0:
            ...
            time_to_go = np.linalg.norm(target_pos - x[0:3]) / np.abs(np.linalg.norm(target_vel)+ avg_vel)
        quad_optimizer.set_time_horizon(time_to_go)
        w_opt = quad_optimizer.run_optimization(initial_state=initial_state,target_pos=target_pos, target_vel=target_vel, return_x=False)
        a1_opt = w_opt[6::9]
        a2_opt = w_opt[7::9]
        a3_opt = w_opt[8::9]
        acc = np.array([a1_opt[0], a2_opt[0], a3_opt[0]])
